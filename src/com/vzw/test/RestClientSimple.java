...
/**
 * Starting an instance of the process, without submitting any variables in a
 * map.
 * 
 */
public class RestClientSimple {
	...
	private static final String DEPLOYMENT_ID = "com.vzw.demo:WorkFlowDemo:1.0";
	private static final String PROCESS_DEF_ID = "WorkFlowDemo.WorkFlowDemoProcess";

	private static String username = "bpmsAdmin";
	private static String password = "rockpet@123";	
	...
	public static String getDataFromService(String urlpath, String method, Map<String, String> params,
			boolean multipart) {
		...
		try {
			...
			if (responseCode >= 200 && responseCode < 300) {
				...
				if (handleException) {
					System.out.println(
							"handleException responseCode:[" + responseCode + "] theMethod.getResponseBodyAsString():["
									+ theMethod.getResponseBodyAsString() + "] urlStr:[" + urlStr + "]");

				} else {

					System.out.println(
							"Unsuccessful responseCode:[" + responseCode + "] theMethod.getResponseBodyAsString():["
									+ theMethod.getResponseBodyAsString() + "] urlStr:[" + urlStr + "]");
